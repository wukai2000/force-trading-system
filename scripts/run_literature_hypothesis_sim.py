...
Does NOT plant a defense_avionics series that the theme map would
auto-convert into ITA/XAR/PPA.
"""
...
def synthetic_proxies(n: int = 120, seed: int = 42):
    ...
    widget = np.linspace(10, 22, n) + rng.normal(0, 0.6, n)
    viral = np.linspace(40, 180, n) + rng.normal(0, 8, n)
    terms = pd.DataFrame({"under_noticed_widget": widget, "viral_headline": viral}, index=idx)
    patents = pd.DataFrame(
        {
            "industrial_robotics": np.concatenate(
                [np.full(80, 10.0), np.linspace(10, 16, n - 80)]
            )
            + rng.normal(0, 0.4, n)
        },
        index=idx,
...
def main() -> int:
    ...
    print("Literature emits themes, not tickets. map_key is not a Force 4 lock.")
    ...
    for h in hyps:
        rec = h.as_dict()
        if rec.get("map_key") in ("defense_sovereign_capacity", "defense_avionics"):
            raise SystemExit(
                f"FAIL: literature simulator auto-mapped {h.model_id} → {rec['map_key']}"
            )
    ...
    payload = {
        ...
        "note": (
            "Hypotheses only. No theme→ticket auto-map. "
            "Run sieve + pit_evaluate.py + pipeline to kill beta."
        ),
        "hypotheses": rows,
    }
